#!/bin/bash
#SBATCH --job-name=prad_pirads_loco_tabfm
#SBATCH --partition=gpuceib
#SBATCH --cpus-per-task=7
#SBATCH --mem=120G
#SBATCH --gres=gpu:1
#SBATCH --output=./logs/prad_pirads_loco_tabfm_%j.out
...
OUTPUT_DIR="results/pirads_experiments/loco/tabfm"
...
python -m pip install "absl-py" "flit_core>=3.8,<4" "jaxtyping<0.3" "typeguard<3" "huggingface-hub"
...
if not torch.cuda.is_available():
    raise SystemExit("PI-RADS LOCO TabFM requested a GPU, but PyTorch does not see CUDA.")
print(f"PyTorch CUDA device: {torch.cuda.get_device_name(0)}")
PY

find_plan() {
  local group="$1"
  mapfile -t plans < <(find "results/pirads_experiments/loco/ml/${group}" -name shared_fold_feature_plan.json -type f)
  if [ "${#plans[@]}" -ne 1 ]; then
    echo "Expected one LOCO feature plan for ${group}; found ${#plans[@]}." >&2
    return 1
  fi
  printf '%s\n' "${plans[0]}"
}
...
run_tabfm() {
  ...
  prostate-radiomics train-tabfm \
    ...
    --shared_feature_folds_json "${plan}" \
    --run_prefix "pirads_loco_${name}" \
    --checkpoint_path "${CHECKPOINT_PATH}" \
    --permutation_importance_repeats 0 \
    --permutation_importance_top_k 0
}

for name in radiomics_pirads radiomics_pirads_clinical; do
  run_tabfm "${name}"
done
...
base = Path("results/pirads_experiments/loco/tabfm")
for name in ["radiomics_pirads", "radiomics_pirads_clinical"]:
    path = base / f"pirads_loco_{name}_tabfm_pretrained/cv_oof_predictions.csv"
    if not path.exists():
        raise SystemExit(f"Missing LOCO TabFM OOF predictions: {path}")
    frame = pd.read_csv(path)
    if len(frame) != 1500 or frame.sample_id.astype(str).nunique() != 1500:
        raise SystemExit(f"Unexpected LOCO TabFM cohort in {path}.")
    if set(frame.fold_index.astype(int)) != {1, 2, 3}:
        raise SystemExit(f"Unexpected LOCO folds in {path}.")
    if set(frame.threshold_source.astype(str)) != {"inner_val_youden"}:
        raise SystemExit(f"Unexpected threshold source in {path}.")
print("Validated two PI-RADS LOCO TabFM experiments.")
PY

echo "PI-RADS LOCO TabFM outputs: ${OUTPUT_DIR}"
