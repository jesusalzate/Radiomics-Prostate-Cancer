...
%% Title and authors
%% ---------------------------------------------------------------
\title{\textbf{Benchmarking Tabular Foundation Models, Classical Machine Learning,
and Deep Tabular Learning for Clinically Significant Prostate Cancer Detection
on Biparametric MRI}}
...
%% ORCID Manuela Mendoza-Barona: 0009-0009-5275-5679
\author[4,9]{Manuela Mendoza-Barona}
...
\author[10]{Mariola Penadés Fons}
\author[4,5,6,7]{Reinel Tabares-Soto}
\author[8]{Juan M García-Gómez}
\author[1,*]{Maria de la Iglesia-Vayá}
\affil[1]{Unidad Mixta de Imagen Biomédica e Inteligencia Artificial FISABIO-CIPF, Fundación para el Fomento de la Investigación Sanitaria y Biomédica de la Comunidad Valenciana, Valencia 46020, Spain}
\affil[2]{Hospital Arnau de Vilanova, Urology, Valencia, 46015, Spain}
\affil[3]{Computational Biomedicine Laboratory, Príncipe Felipe Research Center (CIPF), 46012, Valencia, Spain}
...
\affil[10]{Conselleria de Sanidad, Generalitat Valenciana, Valencia, Spain}
...
detection of clinically significant prostate cancer (csPCa) on biparametric MRI,
and assessed the added value of routine clinical variables. The full PI-CAI
public dataset (1{,}500 studies, 1{,}476 patients; 28.3\% csPCa) was analyzed
under the official patient-grouped five-fold cross-validation, using whole-gland
radiomic features from T2-weighted, apparent diffusion coefficient, and high
b-value diffusion-weighted imaging with a shared, leakage-safe, fold-specific
feature-selection, calibration, and thresholding pipeline; age, prostate-specific
antigen (PSA), PSA density, and prostate volume were evaluated alone, concatenated
with radiomics, and by dual/late fusion. On radiomics alone, TabFM ranked first
(AUROC 0.828, 95\% CI 0.805--0.850) but was not significantly above the calibrated
Random Forest (0.814; Holm-adjusted $P=0.084$) or the deep models (0.794--0.803).
Adding clinical variables produced the best models overall---AUROC 0.844 for TabFM,
whether by concatenation or dual-stream late fusion---a modest but statistically
supported gain over the radiomics-only and previous-best baselines. A random-weight
control collapsed TabFM to near-chance discrimination (AUROC 0.522), indicating that
the gain reflects the foundation model's pretraining rather than its architecture
alone. Under leave-one-center-out retraining, discrimination fell substantially for
every model family (to $\approx$0.68--0.75, an optimistic bias of
$\approx$0.10--0.14 AUROC), and one of three centers was consistently weaker,
showing that aggregate performance masks clinically relevant between-center
variation. Model-agnostic SHAP attributions confirmed a shared diffusion-weighted
and PSA-density signature across model families. A pretrained tabular foundation
model thus led numerically in all conditions, with a modest gain in the combined
settings, while a calibrated classical ensemble remained a highly competitive and
more parsimonious alternative. All comparisons are exploratory, as model families
were screened on the same cohort; the full feature tables and analysis code are
released on this public dataset to provide a reproducible benchmark. This is a
retrospective, internally and leave-one-center-out validated benchmark, not an
externally validated or clinically deployable tool.
\end{abstract}
...
The contributions of this work are therefore fourfold. To our knowledge, this is
the first benchmark of a pretrained tabular foundation model for csPCa radiomics;
it is compared head-to-head with classical and deep tabular learners on an
identical, leakage-safe feature space; discrimination is characterized not only in
aggregate but by contributing center and, critically, under leave-one-center-out
retraining as a realistic test of transportability; and the entire analysis is
performed on a public dataset with the extracted features and code released, so
that the benchmark is fully reproducible and directly extensible.
...
(public weights \texttt{google/tabfm-1.0.0-pytorch}, source commit
\texttt{35a68537}), was added as a fourth model
family alongside the classical ensembles and the deep architectures. For full
reproducibility, TabFM was used in a strictly zero-shot configuration (no
fine-tuning; zero training epochs) with 32 in-context ensemble members and the
inner-validation Youden threshold; predictions are tied to the recorded weights,
source commit, selected features, and run configurations released with the code,
with residual nondeterminism limited to GPU inference and ensemble feature
shuffling. Building on
...
the classical, deep, and TabFM families. To quantify the model-selection
uncertainty introduced by screening several classical algorithms on the evaluation
cohort, a fully nested cross-validation was additionally run in which the winning
classical algorithm was re-selected inside each outer-training fold by grouped inner
cross-validation, so that the choice of algorithm never used outer-fold data; the
resulting out-of-fold predictions therefore embed the selection step in the reported
confidence interval. Three further sensitivity analyses probed
...
imputation); (ii)~a calibration-method comparison replacing Platt scaling with
isotonic regression for the classical radiomics-only and concatenation models,
assessed by the Brier score and the 10-bin expected calibration error (ECE); and
(iii)~the nested algorithm-selection analysis above.
...
per center; to test genuine transportability we retrained every condition---classical,
deep, and TabFM---under a leave-one-center-out (LOCO) protocol, training on two centers
and testing on the entirely held-out third, and pooled the predictions over the three
held-out centers (full results in Supplementary Material, Table~S3 and Figure~S2).
Discrimination fell substantially for every model family: the radiomics-only pooled
LOCO AUROC dropped to 0.702 for TabFM (from 0.828) and 0.695 for the best classical
model (from 0.814), and the combined conditions reached only 0.73--0.75. TabFM retained
a small numerical lead in every LOCO condition, but the families converged markedly
off-center---all seven models fell within a narrow $\approx$0.68--0.75 band, roughly
$0.10$--$0.14$ AUROC below their center-mixed estimates. This optimistic bias is
consistent in magnitude with the between-center gap at PCNN and indicates that the
aggregate five-fold numbers should be read as multicenter internal validation rather
than as estimates of performance at a new site.

Because the classical benchmark had been assembled by screening twelve algorithms on
the same cohort, we also quantified the residual model-selection uncertainty with a
fully nested cross-validation in which the winning classical algorithm was re-chosen
inside each outer-training fold by grouped inner cross-validation and never with
outer-fold data. The nested estimate, which propagates the selection step into the
confidence interval, was statistically indistinguishable from the fixed-model primary
analysis (radiomics-only AUROC 0.811, 95\% CI 0.787--0.833; clinical-only 0.742,
0.715--0.769; concatenation 0.829, 0.807--0.851), and Random Forest was the selected
model in four of five folds in every condition (Gradient Boosting in the fifth;
Supplementary Material, Table~S8). The
per-condition winners are therefore stable and the reported confidence intervals are
not materially widened by model-selection uncertainty, although the screening and the
LOCO/sensitivity contrasts remain exploratory in the formal sense.

The primary conclusions were otherwise robust to the pre-planned sensitivity analyses,
which are reported in full in the Supplementary Material. Restricting the analysis to
the 1{,}049 studies with an originally recorded (non-derived) PSA density left the
ordering and magnitude of results essentially unchanged and, if anything, slightly
higher than the full-cohort imputed analysis (radiomics-only Random Forest AUROC 0.832,
concatenation TabFM 0.873; Table~S4), confirming that the derived and median-imputed
PSA-density values did not inflate the combined-model performance. Discrimination and
model ranking were likewise unchanged when Platt scaling was replaced by isotonic
regression, and the Platt-calibrated models used in the main analysis were already well
calibrated (expected calibration error 0.03--0.05); isotonic regression approximately
halved the expected calibration error at an unchanged Brier score, so the choice of
calibration map does not affect the conclusions (Table~S5, Figure~S3). Feature selection
was stable across folds (mean pairwise Jaccard overlap 0.67--0.69; Table~S6), and at
each model's outer-training-derived operating point the decision-curve analysis
translated into a clinically meaningful reduction in biopsies, with the concatenation
TabFM avoiding 37 biopsies per 100 patients at a negative predictive value of 0.90
(Table~S7).
...
(A)~original DWI slice with prostate gland (cyan) and csPCa lesion (green)
delineations only; (B)~ADC square-root first-order total energy, (C)~DWI
exponential GLCM Imc1, (D)~DWI LoG 3 mm first-order skewness, (E)~DWI wavelet GLCM
Imc2, and (F)~DWI square GLRLM gray-level non-uniformity. Feature panels show only
the voxel map within a prostate-centered bounding box, with the gland and lesion
contours retained. Colour encodes voxel-based feature value on a per-feature
relative scale (low to high). Panel titles indicate the feature direction from the
signed SHAP/integrated-gradient beeswarm analyses: lower values support the
positive csPCa class for B--D and F, whereas higher values support the positive
csPCa class for E.}
...
optimistic bias and severely overestimating performance~\citep{Quality2022}. A further obstacle to
honest comparison is that the great majority of published prostate radiomics models---including the
strongest multimodal ones---were developed on private, single-institution cohorts, frequently with
closed or commercial feature-extraction pipelines, so their results can neither be independently
reproduced nor re-evaluated under a common protocol. By adhering to
TRIPOD+AI guidelines with strictly leakage-safe, patient-grouped cross-validation on the
large, publicly available PI-CAI cohort~\citep{collins2024tripodai,saha2023artificial} and releasing the
full feature tables and analysis code, the present study provides a conservative and, importantly,
\emph{reproducible} internal-validation benchmark against which future models can be directly and
transparently compared---a contribution we regard as central given the reproducibility gap in the
field. Under the implemented protocol, the deep tabular
...
complement to MRI~\citep{RadiomicsMultisite2023,RadiomicsNomogram2022}. The most directly
comparable study is Antol{\'i}n et al., who developed Random Forest models on a single-center
cohort of 1{,}497 examinations and reported that radiomics alone did not outperform PI-RADS
(AUC 0.838 vs.\ 0.833) while a combined radiomics-PI-RADS-clinical model reached
AUC~0.891~\citep{Antolin2026Insights}. Our findings agree closely at the level where the two
studies are commensurable: radiomics-only discrimination was almost identical across the two
otherwise dissimilar cohorts (their 0.838 vs.\ our 0.814--0.828), and Random Forest was the best
classical algorithm in both. The apparent gap between their best model (0.891) and ours (0.844)
should not be read as a weaker radiomic signature: it is largely attributable to their inclusion
of PI-RADS---the single most influential feature in their model and, in our SHAP analysis, an
input we could not access---and to their less conservative single-center random train--test split,
whereas our estimate is a multicenter, patient-grouped cross-validation that degrades further to
$\approx$0.75 under leave-one-center-out retraining. By deliberately omitting PI-RADS to isolate
the whole-gland radiomic signal, our results
...
True external validation is, however, difficult to achieve in this setting: PI-CAI
is among the very few large, multicenter, publicly available prostate-bpMRI cohorts
with histopathological labels and whole-gland annotations, and most studies to
which we might compare rely on private data, so neither a readily available
independent public test set nor a like-for-like reproduction of prior models is
currently straightforward. We
therefore used the largest available public cohort and the strongest internal
surrogate for transportability (leave-one-center-out), and released all features and
code so that the benchmark can be extended the moment additional public cohorts
appear.
...
exceeded---the primary results (Section~\ref{sec:res-transport}; Supplementary
Material, Table~S4), indicating that the imputation did not drive the findings.
...
algorithms and the selection of condition winners reused the evaluation cohort, so
the pairwise comparisons remain exploratory in the formal sense; a fully nested
cross-validation that folds the algorithm-selection step into the confidence
interval, however, reproduced the primary discrimination almost exactly
(Section~\ref{sec:res-transport}) and selected Random Forest in four of five folds
in every condition, indicating that the residual model-selection uncertainty is
small in practice even though independent validation would still be required for
strictly confirmatory inference. TabFM introduces
...
rigorous, reproducible comparative benchmark---not an externally validated or
clinically deployable model---and as evidence that both a relatively simple,
...
\textbf{Investigation:} J.A.A.-G., M.A.B.-O., L.M.C.P., S.L.M., M.M.-B.
...
This work is developed in the context of broader European efforts on the clinical
implementation of imaging- and AI-based diagnostic tools, including the JANE-2 EU
Joint Action on the implementation of omics and advanced diagnostics in clinical
practice (\url{https://jane-2.eu/})~\citep{GuerraNunez2026Epigenomics}.
