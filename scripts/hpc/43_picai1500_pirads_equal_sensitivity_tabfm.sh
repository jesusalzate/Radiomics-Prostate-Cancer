#!/bin/bash
#SBATCH --job-name=prad_pirads_eqsens_tabfm
#SBATCH --partition=gpuceib
#SBATCH --cpus-per-task=7
#SBATCH --mem=120G
#SBATCH --gres=gpu:1
#SBATCH --output=./logs/prad_pirads_eqsens_tabfm_%j.out
...
OUTPUT_DIR="results/pirads_experiments/equal_sensitivity/tabfm"
...
if not torch.cuda.is_available():
    raise SystemExit("Equal-sensitivity TabFM job requested a GPU, but PyTorch does not see CUDA.")
...
run_tabfm() {
  ...
  prostate-radiomics train-tabfm \
    ...
    --run_prefix "equal_sensitivity_${name}" \
    --checkpoint_path "${CHECKPOINT_PATH}" \
    --target_sensitivities 0.9811764705882353 0.9364705882352941 \
...
base = Path("results/pirads_experiments/equal_sensitivity/tabfm")
targets = ["0p981176", "0p936471"]
for name in ["pirads_psad", "radiomics_pirads", "radiomics_pirads_clinical"]:
    path = base / f"equal_sensitivity_{name}_tabfm_pretrained/cv_oof_predictions.csv"
    ...
    if set(frame.threshold_source.astype(str)) != {"inner_val_youden"}:
        raise SystemExit(f"Unsafe or unexpected threshold source in {path}.")
    for target in targets:
        for prefix in ["threshold", "prediction"]:
            column = f"{prefix}_target_sensitivity_{target}"
            if column not in frame:
                raise SystemExit(f"Missing {column} in {path}.")
print("Validated three equal-sensitivity TabFM OOF experiments.")
PY

echo "Equal-sensitivity TabFM outputs: ${OUTPUT_DIR}"
