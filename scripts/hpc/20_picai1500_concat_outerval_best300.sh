#!/bin/bash
#SBATCH --job-name=prad_p1500_concatbest300
#SBATCH --partition=gpuceib
#SBATCH --cpus-per-task=7
#SBATCH --mem=100G
#SBATCH --gres=gpu:1
#SBATCH --output=./logs/prad_p1500_concatbest300_%j.out
...
RUN_ROOT="${BASE_DIR}/dl/concat_outerval_best300"
RUN_PREFIX="picai1500_concat_outerval_best300_5fold"
...
prostate-radiomics train-deep \
  --config configs/experiments/picai1500_corr/concat_outerval_best300.yaml

prostate-radiomics postprocess-deep \
  --config configs/experiments/picai1500_corr/concat_outerval_best300_postprocess.yaml

prostate-radiomics interpret \
  --config configs/reports/picai1500_corr/concat_outerval_best300_benchmark.yaml \
  -- \
...
python scripts/analysis/compare_concat_outerval_best300.py
...
for architecture in ("transformer", "capsnet", "transformer_capsnet"):
    ...
    for fold in summary.get("folds", []):
        ...
        if (
            split_sizes.get("train") + split_sizes.get("validation") != 1500
            or split_sizes.get("validation") != split_sizes.get("test")
        ):
            raise SystemExit(f"{run_dir}/{fold.get('fold_label')} has invalid split sizes")
...
if set(pooled["model_name"].astype(str)) != expected_models:
    raise SystemExit("The benchmark does not contain the expected six models")
...
print("Validated radiomics+clinical full-300 best-epoch OOF sensitivity experiment.")
PY

echo "Radiomics+clinical best-of-300 comparison complete: ${RUN_ROOT}"
