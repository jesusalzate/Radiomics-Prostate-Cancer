#!/bin/bash
#SBATCH --job-name=prad_p1500_voxrender
#SBATCH --partition=gpuceib
#SBATCH --cpus-per-task=7
#SBATCH --mem=120G
#SBATCH --gres=gpu:0
#SBATCH --output=./logs/prad_p1500_voxrender_%j.out
...
: "${PATIENT_ID:?Set PATIENT_ID, e.g. sbatch --export=ALL,PATIENT_ID=10233,STUDY_ID=1000237 scripts/hpc/09_render_signed_top20_maps_only.sh}"
: "${STUDY_ID:?Set STUDY_ID, e.g. sbatch --export=ALL,PATIENT_ID=10233,STUDY_ID=1000237 scripts/hpc/09_render_signed_top20_maps_only.sh}"
...
echo "PI-CAI 1500 voxel evidence maps rebuilt under: ${OUTPUT_DIR}/${PATIENT_ID}_${STUDY_ID}"
